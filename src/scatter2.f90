...
real(rk),allocatable:: x(:),ang_cos(:),e(:),ang_weight(:) ! the grid points in x, angle and energy
...
do j=1,na 
do i=1,nx 
T_ABC(i,j)=j*(j-1)/2.0_rk/massBC/x(i)**2
end do
end do
!$omp end parallel do 
...
do i=1,na 
WF_A(i) = sqrt(ang_weight(i))*yjm0(j0,k0,ang_cos(i))
enddo 
!$omp end parallel do 
...
use amod_propatation,only:split,T2E 
use Math_Functions,only:Gamma
integer::i,j,k,it,ie
real(rk)::ting,poping
...
time: do while(poping>1e-5)
  ...
  !split(nstate,n,l,m,dt,T_rad,T_rot,Vs,U_rad,U_ang,wfd,wff)
  call split(nstate,nx,na,dt,Tx,TABC,Vs,VsC,U_rad,U_ang,wfing,WFtemp)
